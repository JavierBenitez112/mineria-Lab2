...
## Paso 4: Generación de reglas (Primer intento)
...
## Paso 6: Filtrado de reglas relevantes

```{r reglas_filtradas}
reglas_filtradas <- subset(reglas_ordenadas, lift > 1.2)
...
## Paso 8: Interpretación de reglas

Las reglas más interesantes son aquellas que presentan:

- Soporte suficiente (no demasiado bajo)
- Confianza alta
- Lift mayor que 1 (idealmente > 1.2)

El lift indica dependencia real entre los ítems. 
Un lift > 1 significa que la ocurrencia conjunta es mayor a la esperada si fueran independientes.

Reducir el soporte permitió encontrar relaciones más específicas entre géneros, mientras que mantener una confianza moderada evitó reglas débiles.

Se deben analizar especialmente las reglas con mayor lift, ya que representan asociaciones estructuralmente fuertes dentro del mercado cinematográfico.
